class DailyDeal::Deal 
  ...
  def self.today 
    # Scrape woot and meh nd then return deals based on that data 
    self.scrape_deals 
  end
  ...
  def self.scrape_woot
    ...
    deal.url = doc.search(".wantone").first.attr("href").strip
  ...
  def self.scrape_meh
    ...
    deal = self.new 
    deal.name = title = doc.search("section.features h2").text.strip
